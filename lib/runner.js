    ...
    const findEntry = (selectorList) => {
      for (const selector of parseSelectors(selectorList)) {
        for (const entry of entries) {
          const elements = Array.from(entry.doc.querySelectorAll(selector));
...
async function getNativeManualChallengeState(tabId, challengeConfig = {}) {
  return evaluateWithDebuggerDomHelpers(tabId, {
    ...
    const continueEntry = findByTexts(
    ...
    return {
      ...
      continueButton: describeEntry(continueEntry),
...
export async function runModule(module, moduleState, options = {}) {
  ...
  try {
    ...
    const runManualChallenge = async (challengeConfig, pageId = 'verification', pageLabel = 'Verification') => {
      ...
      try {
        const nativePreparationResult = await runNativeManualChallengePreparation(tab, challengeConfig);
      ...
      const challengeResult = chooseChallengeResult(assistResults);
      pageRuns.push({
        pageId,
        pageLabel,
        ...challengeResult
    ...
    for (const page of module.pages) {
      ...
      }, page.id), page.executionTimeoutMs || (page.waitFor?.timeoutMs || 15000) + 5000, `${module.displayName} ${page.label}`);
