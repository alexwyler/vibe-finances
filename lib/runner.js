...
async function waitForNativeSelector(tabId, selectorList, timeoutMs = 10000) {
  if (!selectorList) {
    return true;
  }
  ...
  while (Date.now() - startedAtMs < timeoutMs) {
    ...
    const result = await evaluateWithDebuggerDomHelpers(tabId, {
      selectors: selectorList
    }, `
      return { found: Boolean(findEntry(payload.selectors)) };
    `);
...
async function runNativeLoginPreActions(tabId, loginConfig) {
  for (const action of loginConfig.preActions || []) {
    ...
    const locator = await locateNativeElement(
      tabId,
      action.selector || 'a, button, [role="button"]',
      action.text
        ? {
          text: action.text,
    ...
    if (!locator?.found) {
      return {
        ok: false,
        message: `Could not find pre-login click target: ${action.selector || action.text || 'click target'}`
      };
    }
    ...
    if (action.waitFor) {
      const ready = await waitForNativeSelector(tabId, action.waitFor, action.timeoutMs || loginConfig.timeoutMs || 10000);
      if (!ready) {
        return {
          ok: false,
          message: `Timed out waiting for login pre-action selector: ${action.waitFor}`
        };
      }
    }
...
async function attemptTrustedSubmitSequenceAttached(tabId, loginConfig, nativeConfig, stage = 'final') {
  ...
  return {
    ...
    message: stage === 'username'
      ...
      : 'Trusted submit attempts did not advance the login page.'
  };
}
...
async function runNativeLoginFlow(tab, loginConfig, username, password) {
  ...
  try {
    return await withAttachedDebugger(tab.id, async () => {
      const preActionsResult = await runNativeLoginPreActions(tab.id, loginConfig);
      if (!preActionsResult.ok) {
        return {
          ok: false,
          foundForm: false,
          message: preActionsResult.message
        };
      }
      ...
      if (!state?.username?.found && !state?.password?.found) {
        return {
          ok: false,
          foundForm: false,
          message: 'Trusted login flow could not find the username/password fields.'
        };
      }
      ...
      if (!state?.password?.found) {
        return {
          ok: false,
          foundForm: true,
          message: 'Trusted login flow could not reach the password field.'
        };
      }
  ...
  return (async () => {
    ...
    for (const extractor of payload.extractors) {
      try {
        ...
        if (extractor.kind === 'currencyNearText') {
          // Smallest visible element whose text contains one of the labels and a dollar amount,
          // polled until it renders.
          const labels = (extractor.textIncludes || []).map((value) => String(value).toLowerCase());
          const currencyPattern = /\$\s?[0-9][0-9,]*(?:\.[0-9]{2})?/;
          const findMatch = () => {
            const candidates = queryAll(document, extractor.itemSelector || 'body *')
              .filter((element) => {
          ...
          let item = findMatch();
          const deadlineMs = Date.now() + (extractor.timeoutMs || 0);
          while (!item && Date.now() < deadlineMs) {
            await sleep(500);
            item = findMatch();
          }

          const valueText = item ? (normalizeText(item.textContent).match(currencyPattern)?.[0] || '') : '';
          values[extractor.id] = {
            type: 'currency',
            label: extractor.label,
            valueText,
            valueNumber: parseCurrency(valueText),
            meta: item ? { title: normalizeText(item.textContent).slice(0, 120) } : {}
          };
          continue;
        }
...
export async function runModule(module, moduleState, options = {}) {
  ...
  try {
    ...
    // For sites whose 2FA lives on its own page: if the ready state isn't reached after login,
    // bring the tab forward and wait for the user to finish verification themselves.
    const waitForUserVerification = async (verificationConfig) => {
      const checkReady = async () => {
        ...
        try {
          const readyState = await executeInTab(tab.id, injectedAdvancePostChallengePage, [verificationConfig]);
          return Boolean(readyState?.ready);
        } catch (error) {
          // The page is mid-navigation; try again on the next poll.
          return false;
        }
      };

      const initialCheckMs = verificationConfig.initialCheckMs || 8000;
      const pollMs = verificationConfig.pollMs || 500;
      const initialStartedAtMs = Date.now();
      while (Date.now() - initialStartedAtMs < initialCheckMs) {
        if (await checkReady()) {
          return;
        }
        await sleep(pollMs);
      ...
      pageRuns.push({
        ...
        ok: true,
        awaitingUser: true,
        message: verificationConfig.message || `Complete ${module.displayName} verification in the opened tab.`
      });

      if (typeof options.onPageResult === 'function') {
        try {
          await options.onPageResult({
            ok: false,
            inProgress: true,
            moduleId: module.id,
            displayName: module.displayName,
          ...
          console.warn('Could not persist verification progress.', error);
        }
      }
      ...
      try {
        const timeoutMs = verificationConfig.timeoutMs || 300000;
        const startedAtMs = Date.now();
        while (Date.now() - startedAtMs < timeoutMs) {
          if (await checkReady()) {
            if (verificationConfig.settleMs) {
              await sleep(verificationConfig.settleMs);
            }
            return;
          }
          await sleep(pollMs);
        ...
        throw new Error(`${module.displayName} verification timed out waiting for the code to be entered.`);
      } finally {
        if (previousActiveTabId && previousActiveTabId !== tab.id) {
          try {
            const [activeTab] = await chrome.tabs.query({ active: true, windowId: currentTab.windowId });
    ...
    // A session that is still signed in stays on the start page instead of redirecting to login.
    const isAlreadySignedIn = async (signedInConfig) => {
      if (!signedInConfig?.urls?.length) {
        return false;
      }

      const checkMs = signedInConfig.checkMs || 6000;
      const stableMs = signedInConfig.stableMs || 3000;
      const startedAtMs = Date.now();
      let matchedSinceMs = null;
      while (Date.now() - startedAtMs < checkMs) {
        const currentTab = await chrome.tabs.get(tab.id);
        if (urlMatchesAnyTarget(currentTab.url, signedInConfig.urls)) {
          matchedSinceMs = matchedSinceMs || Date.now();
          if (Date.now() - matchedSinceMs >= stableMs) {
            return true;
          }
        } else {
          matchedSinceMs = null;
          if (urlMatchesAnyTarget(currentTab.url, signedInConfig.loginUrls || [])) {
            return false;
          }
        }
        await sleep(250);
    ...
    if (module.login && await isAlreadySignedIn(module.login.alreadySignedIn)) {
      pageRuns.push({
        pageId: 'login',
        pageLabel: 'Login',
        ok: true,
        foundForm: false,
        message: 'Already signed in; skipped login.'
      });
    ...
    if (module.login?.awaitUserVerification) {
      await waitForUserVerification(module.login.awaitUserVerification);
    }
    ...
    for (const page of module.pages) {
      ...
      const maxAttempts = 1 + Math.max(0, page.scrapeRetries || 0);
      ...
      while (attempt < maxAttempts) {
        ...
        if (attempt > 1) {
          try {
            ...
            console.warn(`Could not re-navigate before retrying ${module.displayName} ${page.label}.`, error);
          }
        }

        try {
          scrapeResult = await withTimeout(runInteraction(async () => {
        ...
        if (scrapeResult?.ok || attempt >= maxAttempts) {
          break;
        }
      }
      ...
      if (typeof options.onPageResult === 'function') {
        try {
          ...
          console.warn('Could not persist page progress.', error);
        }
      }
    }
