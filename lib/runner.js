...
function injectedRunLoginPreActions(payload = {}) {
  const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
  const normalizeText = (value) => (value || '').replace(/\s+/g, ' ').trim();
  const getElementText = (element) => normalizeText(
    element?.innerText
  ...
  const waitForSelector = async (selectorList, timeoutMs = 15000, options = {}) => {
    const start = Date.now();
    while (Date.now() - start < timeoutMs) {
      const element = options.visible === true ? queryFirstVisible(selectorList) : queryFirst(selectorList);
      if (element) {
        return element;
      }
      await sleep(250);
  ...
  return (async () => {
    for (const action of payload.preActions || []) {
      ...
      const selector = action.selector || 'a, button, [role="button"]';
      const target = action.text
        ? queryByText(selector, action.text, {
          match: action.textMatch,
          visible: action.visible
        })
        : (action.visible === false ? queryFirst(selector) : queryFirstVisible(selector));
    ...
    const runLoginAttempt = async (loginConfig, pageId = 'login', pageLabel = 'Login') => {
      const runConfiguredLogin = async () => {
        if (loginConfig.nativeLoginFlow?.enabled) {
          if (loginConfig.preActions?.length) {
            await runInteraction(() => executeInTab(tab.id, injectedRunLoginPreActions, [{
              preActions: loginConfig.preActions
