...
function injectedLogin(payload) {
  ...
  const setValueIfPresent = async (selectorList, value) => {
    ...
    const element = await waitForSelector(selectorList, login.timeoutMs || 15000);
    if (!element) {
      return false;
    }
  ...
  return (async () => {
    ...
    const usernameInput = requiresUsername
      ? await waitForSelector(login.selectors.username, login.timeoutMs || 15000)
      : queryFirst(login.selectors.username);
...
export async function runModule(module, moduleState, options = {}) {
  ...
  try {
    ...
    if (module.login) {
      const loginResults = await executeInTab(tab.id, injectedLogin, [{
        login: module.login,
        username,
        password
      }], { allFrames: true });
      const loginResult = chooseLoginResult(loginResults);
      pageRuns.push({
        pageId: 'login',
        pageLabel: 'Login',
        ...loginResult
