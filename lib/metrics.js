...
const LEGACY_CASHFLOW_EXTRACTOR_IDS = {
  income: 'cashflowIncome',
  fixed: 'cashflowFixed',
  discretionary: 'cashflowDiscretionary'
};

const CASHFLOW_CHECKING_EXTRACTOR_ID = 'wellsFargoChecking';
const CASHFLOW_CREDIT_CARD_EXTRACTOR_ID = 'chaseCreditCardBalance';
...
function getCashflowRole(extractor, pageId) {
  ...
  if (extractor.id === LEGACY_CASHFLOW_EXTRACTOR_IDS.income) {
    return 'income';
  }

  if (extractor.id === LEGACY_CASHFLOW_EXTRACTOR_IDS.fixed) {
    return 'fixed';
  }

  if (extractor.id === LEGACY_CASHFLOW_EXTRACTOR_IDS.discretionary) {
...
function getEnabledExtractorDefinition(state, extractorId) {
  for (const module of MODULES) {
...
function getCurrentCurrencyExtractorValue(state, extractorId) {
  const definition = getEnabledExtractorDefinition(state, extractorId);
  if (!definition) {
    return null;
  }

  const { result, isStale } = getEffectiveExtractorResult(state, definition.module.id, extractorId);
...
function getCurrencyExtractorSnapshotPoints(source, extractorId) {
  return getSnapshots(source)
    .map((snapshot) => ({
    ...
    .filter((point) => typeof point.timestamp === 'number' && !Number.isNaN(point.timestamp));
}

function getOldestCurrencyExtractorValueInWindow(source, extractorId, windowEndTimestamp, currentPoint = null) {
  const windowStartTimestamp = windowEndTimestamp - MONTH_MS;
  const points = getCurrencyExtractorSnapshotPoints(source, extractorId);
  ...
  const oldestPoint = points
    .filter((point) => point.timestamp >= windowStartTimestamp && point.timestamp <= windowEndTimestamp)
    .sort((left, right) => left.timestamp - right.timestamp)[0];

  return typeof oldestPoint?.value === 'number' && !Number.isNaN(oldestPoint.value)
    ? oldestPoint.value
    : null;
}
...
function computeCheckingDeltaCashflowDetails(state, timestamp = Date.now()) {
  const checking = getCurrentCurrencyExtractorValue(state, CASHFLOW_CHECKING_EXTRACTOR_ID);
  const creditCard = getCurrentCurrencyExtractorValue(state, CASHFLOW_CREDIT_CARD_EXTRACTOR_ID);
  const baselineCheckingValue = checking
    ? getOldestCurrencyExtractorValueInWindow(
      state,
      CASHFLOW_CHECKING_EXTRACTOR_ID,
      timestamp,
      {
        timestamp,
  ...
  if (
    !checking
    || !creditCard
    || typeof baselineCheckingValue !== 'number'
    || Number.isNaN(baselineCheckingValue)
  ) {
    return null;
  }

  return {
    value: checking.value - baselineCheckingValue - creditCard.value,
    isStale: checking.isStale || creditCard.isStale
  };
}

function computeHistoricalCheckingDeltaCashflow(state, targetTimestamp) {
  if (
    !getEnabledExtractorDefinition(state, CASHFLOW_CHECKING_EXTRACTOR_ID)
    || !getEnabledExtractorDefinition(state, CASHFLOW_CREDIT_CARD_EXTRACTOR_ID)
  ) {
    return null;
  }

  const checkingValue = getInterpolatedExtractorValue(state, CASHFLOW_CHECKING_EXTRACTOR_ID, targetTimestamp);
  const baselineCheckingValue = getOldestCurrencyExtractorValueInWindow(
    state,
    CASHFLOW_CHECKING_EXTRACTOR_ID,
    targetTimestamp
  );
  const creditCardValue = getInterpolatedExtractorValue(state, CASHFLOW_CREDIT_CARD_EXTRACTOR_ID, targetTimestamp);

  if (
    typeof checkingValue !== 'number'
    || Number.isNaN(checkingValue)
    || typeof baselineCheckingValue !== 'number'
    || Number.isNaN(baselineCheckingValue)
    || typeof creditCardValue !== 'number'
    || Number.isNaN(creditCardValue)
  ) {
    return null;
  }

  return checkingValue - baselineCheckingValue - creditCardValue;
}

export function computeGlobalMetricDetails(state, timestamp = Date.now()) {
  let netWorth = 0;
  let income = 0;
  let fixed = 0;
  let discretionary = 0;
  ...
  forEachEnabledExtractor(state, ({ module, pageId, extractor }) => {
    ...
    const cashflowRole = getCashflowRole(extractor, pageId);

    if (cashflowRole === 'income') {
      income += result.valueNumber;
      incomeStale = incomeStale || isStale;
      return;
    }

    if (cashflowRole === 'fixed') {
      fixed += result.valueNumber;
      fixedStale = fixedStale || isStale;
      return;
    }

    if (cashflowRole === 'discretionary') {
  ...
  const checkingDeltaCashflow = computeCheckingDeltaCashflowDetails(state, timestamp);
  ...
  return {
    ...
    netCashflow: checkingDeltaCashflow || {
      value: 0,
      isStale: false
    }
  };
...
export function computeGlobalMetrics(state, timestamp = Date.now()) {
  const metricDetails = computeGlobalMetricDetails(state, timestamp);
  return {
...
export function computeHistoricalMetrics(state, targetTimestamp) {
  ...
  const checkingDeltaCashflow = computeHistoricalCheckingDeltaCashflow(state, targetTimestamp);
