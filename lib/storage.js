...
export function mergeStateWithDefaults(storedState = {}) {
  ...
  merged.runStatus = {
    modules: {
      ...(storedState.runStatus?.modules || {})
    }
  };
  merged.history = {
    snapshots: Array.isArray(storedState.history?.snapshots) ? storedState.history.snapshots : []
  };
  merged.runLog = Array.isArray(storedState.runLog) ? storedState.runLog : [];
...
export async function getState() {
  const { state } = await chrome.storage.local.get('state');
  return mergeStateWithDefaults(state);
}
...
export async function mergeResultValues(moduleId, partialResult) {
  const state = await getState();
  const currentResult = state.results?.[moduleId] || {};
...
export async function updateModuleRunStatus(moduleId, status) {
  const state = await getState();
  state.runStatus = state.runStatus || { modules: {} };
  state.runStatus.modules = {
    ...(state.runStatus.modules || {}),
    [moduleId]: {
      ...(state.runStatus.modules?.[moduleId] || {}),
      ...status,
...
export async function appendRunLog(entries) {
  ...
  const state = await getState();
  state.runLog = [...(state.runLog || []), ...normalizedEntries].slice(-MAX_RUN_LOG_ENTRIES);
  await chrome.storage.local.set({ state });
  return state;
}
...
export async function appendHistorySnapshot(timestamp = Date.now()) {
  const state = await getState();
  const metrics = computeGlobalMetrics(state, timestamp);
  const extractors = {};
...
export function dropInvalidExtractorHistory(state) {
  ...
  for (const module of MODULES) {
    for (const extractor of flattenExtractors(module)) {
      const invalidBetween = extractor.historyInvalidBetween;
      if (!Array.isArray(invalidBetween) || invalidBetween.length !== 2) {
        continue;
      }

      const startMs = Date.parse(invalidBetween[0]);
      const endMs = Date.parse(invalidBetween[1]);
      if (!Number.isFinite(startMs) || !Number.isFinite(endMs) || startMs >= endMs) {
        continue;
      }

      for (const snapshot of snapshots) {
        if (typeof snapshot.timestamp !== 'number' || snapshot.timestamp < startMs || snapshot.timestamp >= endMs) {
          continue;
        }

        if (snapshot.extractors && snapshot.extractors[extractor.id] !== undefined) {
          delete snapshot.extractors[extractor.id];
          changed = true;
        }
      }
    }
...
export async function purgeInvalidExtractorHistory() {
  const state = await getState();
  if (dropInvalidExtractorHistory(state)) {
    await chrome.storage.local.set({ state });
  }
  return state;
}
